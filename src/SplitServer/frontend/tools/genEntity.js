...
function filterKeys(obj, appendFilter) {
  return Object.keys(obj).filter(
    (key) =>
      !key.includes("$") &&
      !key.includes("@") &&
      (appendFilter == undefined || appendFilter(key))
  );
}
...
function asyncGet(url) {
  return new Promise((resolve, reject) => {
    ...
    http.get(options, (res) => {
      ...
      res.on("data", (body) => {
        if (str) {
          str = Buffer.concat([str, body]);
        } else {
          str = Buffer.from(body);
        }
      });
      res.on("end", () => resolve(str.toString()));
      res.on("error", () => reject());
    });
  });
}
...
async function genEntity(obj) {
  ...
  const fd = fs.openSync(
    path.join(path.dirname(__dirname), "src", "defs", "Entity.ts"),
    "w"
  );
  const getPropertyStr = (name, meta) => {
    ...
    if (type == undefined) {
      ...
      const isEnum =
        obj[ns] != undefined &&
        obj[ns][entityName] != undefined &&
        obj[ns][entityName]["$Kind"] === "EnumType";
      if (numberTypes.includes(type)) {
        ret += `    ${name}?: number;${os.EOL}`;
      } else if (type == "Edm.DateTimeOffset") {
        ret += `    ${name}?: MomentInput;${os.EOL}`;
        ...
        ret += `    ${name}?: ${
          isArray ? `Array<${tmp[tmp.length - 1]}>` : `${tmp[tmp.length - 1]}`
        };${os.EOL}`;
      } else if (isEnum) {
  ...
  fs.writeSync(fd, `import { MomentInput } from "moment";${os.EOL}`);
    ...
    .forEach((k) => {
      ...
      Object.keys(ns).forEach((typeName) => {
        ...
        if (desc) {
          fs.writeSync(
            fd,
            `/**${os.EOL} *${desc.replaceAll("\r\n", "")}${os.EOL} */${os.EOL}`
          );
        }
        const typeKind = type["$Kind"];
        entityNames.push(typeName);
        if (typeKind === "EnumType") {
          const value = filterKeys(type)
            .map((tk) => `"${tk}"`)
            .join(" | ");
          fs.writeSync(
            fd,
            `export declare type ${typeName} = ${value};${os.EOL}`
          );
        } else if (typeKind === "ComplexType") {
          fs.writeSync(
            fd,
            `export declare class ${typeName} extends BizView {${os.EOL}`
          );
          filterKeys(type)
            .map((k) => getPropertyStr(k, type[k]))
            .forEach((p) => {
              fs.writeSync(fd, p);
            });
          fs.writeSync(fd, `}${os.EOL}`);
          ...
          fs.writeSync(
            fd,
            `export declare class ${typeName} extends BizEntity {${os.EOL}`
          );
          filterKeys(type, (k) => k != "Id")
            .map((k) => getPropertyStr(k, type[k]))
            .forEach((p) => {
              fs.writeSync(fd, p);
            });
...
function genZhAuditEntity(meta) {
  ...
  fs.writeSync(fd, `        User: "用户"${os.EOL}`);
  fs.writeSync(fd, `    },${os.EOL}`);
  fs.writeSync(fd, `    title: {${os.EOL}`);
  fs.writeSync(fd, `        Type: "类型",${os.EOL}`);
  fs.writeSync(fd, `        TableView: "数据记录",${os.EOL}`);
  fs.writeSync(fd, `        HistoryView: "历史记录"${os.EOL}`);
  fs.writeSync(fd, `    },${os.EOL}`);
  ...
  Object.keys(meta)
    ...
    .forEach((ns) => {
      Object.keys(meta[ns])
        .filter((entityName) => {
          return entityName.endsWith("Audit");
        })
        .forEach((entityName) => {
          processedCnt++;
          ...
          Object.keys(e)
            .filter(
              (propertyName) =>
                !propertyName.includes("$") && !propertyName.includes("@")
            )
            .forEach((propertyName, propertyIdx, propertyNames) => {
              ...
              if (type) {
                fs.writeSync(
                  fd,
                  `                    Type: "${propertyMeta["$Type"]}"${os.EOL}`
                );
              }
              fs.writeSync(
                fd,
                propertyIdx == propertyNames.length - 1
                  ? `                }${os.EOL}`
                  : `                },${os.EOL}`
              );
            });
...
function genEnAuditEntity(meta) {
  const fd = fs.openSync(
    path.join(path.dirname(__dirname), "src", "i18n", "en", "AuditRecord.js"),
    "w"
  );
  ...
  fs.writeSync(fd, `        User: "User"${os.EOL}`);
  fs.writeSync(fd, `    },${os.EOL}`);
  fs.writeSync(fd, `    title: {${os.EOL}`);
  fs.writeSync(fd, `        Type: "Type",${os.EOL}`);
  fs.writeSync(fd, `        TableView: "Data Record",${os.EOL}`);
  fs.writeSync(fd, `        HistoryView: "History Record"${os.EOL}`);
  fs.writeSync(fd, `    },${os.EOL}`);
  fs.writeSync(fd, `    actions: {${os.EOL}`);
...
async function genControllerData(entityNames) {
  ...
  const urls = Object.keys(obj.paths).filter(
    (p) => !p.startsWith("/OData") && !p.startsWith("/ExportCsv")
  );

  fs.readdirSync(path.join(path.dirname(__dirname), "src", "api")).forEach(
    (file) => {
  ...
  const getTypeString = (type, format) => {
    if (numberTypes.includes(type)) {
      ...
      return `${getTypeString(type.substring(0, type.length - 2), format)}[]`;
    } else if (type.toUpperCase() === "OBJECT") {
      // 泛型？
      if (numberTypes.includes(format)) {
  ...
  const getParameterFromRef = (refStr) => {
    ...
    // TODO 太屎了
    if (parameterType == "Result<Int64>") {
      return "Result<number>";
    }
    return parameterType == "TimeSpan" ? "string" : parameterType;
  };
  urls.forEach((url) => {
    const urlSplit = url
      .split("/")
    ...
    if (apiName.includes("-")) {
      apiName = apiName
        ...
        .reduce(
          (p, c, idx) =>
            p +
            (idx == 0 ? c.substring(0, 1) : c.substring(0, 1).toUpperCase()) +
            c.substring(1),
          ""
        );
    }
    apiMethods.forEach((method) => {
      ...
      const methodRequestRef = apiInfo[method].requestBody?.content
        ? apiInfo[method].requestBody?.content["application/json"]?.schema[
            "$ref"
          ]
        : undefined;
      const methodResponseRef = apiInfo[method].responses["200"]?.content
        ? apiInfo[method].responses["200"]?.content["application/json"]?.schema[
            "$ref"
          ]
        : undefined;
      ...
      const meta = {
        ...
        method,
        parameters:
          apiInfo[method].parameters === undefined
            ? []
            : apiInfo[method].parameters.filter((p) => p.in !== "header"),
        name: `${method2BizName(method)}${apiName
          .substring(0, 1)
          .toUpperCase()}${apiName.substring(1)}`, // : apiName
      };
      if (methodRequestRef) {
        meta.request = getParameterFromRef(methodRequestRef);
      } else if (
        apiInfo[method].requestBody?.content["multipart/form-data"] !==
        undefined
      ) {
  ...
  const getUrlParameterStr = (parameters) => {
    ...
    if (parameters !== undefined && parameters.length > 0) {
      return parameters
        .sort((a, b) =>
          a.required === b.required ? 0 : a.required === true ? -1 : 1
        )
        .map(
          (p) =>
            `${p.name}${
              p.required === true ? "" : "?"
            }: ${getUrlParameterTypeStr(p.schema)}`
        )
        .join(", ");
    } else {
      return "";
  ...
  const inserApiModelIfNotExist = (modelType) => {
    if (modelType === "Result" || modelType === "string") {
      return;
    }
    ...
    if (
      !apiModels.some((m) => m.name === modelType) &&
      !entityNames.includes(modelType)
    ) {
      const model = Object.assign(
        { name: modelType },
        obj.components.schemas[modelType]
      );
  ...
  Object.keys(apiMetadata).forEach((module) => {
    ...
    const fd = fs.openSync(
      path.join(path.dirname(__dirname), "src", "api", `${module}_gen.ts`),
      "w"
    );
    ...
    fs.writeSync(
      fd,
      `import { ${[...new Set(importRequest)].join(
        ", "
      )} } from "@/utils/request";${os.EOL}`
    );
    ...
    moduleMeta.forEach((apiMeta) => {
      if (apiMeta.parameters) {
        apiMeta.parameters.forEach((par) => {
          if (par.schema && par.schema["$ref"]) {
            moduleModels.push(par.schema["$ref"]);
          }
        });
      }
    });
    ...
    [
      ...new Set(moduleModels.filter((m) => m !== undefined && m.length > 0)),
    ].forEach((mm) => {
      convertType(mm)
        .map((cm) => (cm.endsWith("[]") ? cm.substring(0, cm.length - 2) : cm))
        .filter(
          (cm) => obj.components.schemas.hasOwnProperty(cm) || cm === "Result"
        )
        .forEach((cm) => {
    ...
    if (moduleApiModels.length > 0) {
      ...
      if (importRequest.includes("request")) {
        fs.writeSync(
          fd,
          `import { ${moduleApiModels.join(", ")} } from "@/defs/Model";${
            os.EOL
          }`
        );
      }
    ...
    if (moduleEntityModels.length > 0) {
      fs.writeSync(
        fd,
        `import { ${moduleEntityModels.join(", ")} } from "@/defs/Entity";${
          os.EOL
        }`
      );
    }
    ...
    moduleMeta.forEach((apiMeta) => {
      ...
      const queryParameters = apiMeta.parameters.filter(
        (p) => p.in === "query"
      );
      const requestRaw = apiMeta.responseRaw || apiMeta.requestRaw;
      let functionName = apiMeta.name;
      let idx = 1;
      while (functionNames.includes(functionName)) {
        functionName = `${apiMeta.name}${idx}`;
        idx += 1;
      }
      functionNames.push(functionName);

      const requestApi = requestRaw
        ? "requestRaw"
        : `request.request<${apiMeta.response}, ${apiMeta.response}>`;
      fs.writeSync(
        fd,
        `${os.EOL}export function ${functionName}(${
          apiMeta.request
            ? `data: ${apiMeta.request}${parameterStr.length > 0 ? ", " : ""}`
            : ""
        }${parameterStr}) {${os.EOL}`
      );
      if (queryParameters.length > 0) {
        ...
        queryParameters.forEach((par) => {
          if (par.required === true) {
            fs.writeSync(
              fd,
              `  query.push(\`${par.name}=\${${par.name}}\`);${os.EOL}`
            );
          } else {
        ...
        fs.writeSync(
          fd,
          `  const queryStr = query.length > 0 ? \`?\${query.join(\"&\")}\` : \"\";${os.EOL}`
        );
      }
      const methodContent = [
        queryParameters.length > 0
          ? `url: \`${apiMeta.url}\$\{(queryStr.length > 0 ? queryStr : "")\}\``
          : `url: \`${apiMeta.url}\``,
  ...
  const getPropertyTypeStr = (property) => {
    if (property.type === "array") {
      ...
      const itemType = getParameterFromRef(property["$ref"]);
      if (/<Nullable<\w+>>/.test(itemType) && /<\w+>/.test(itemType)) {
        let inner = /<\w+>/.exec(itemType)[0];
        let outer = itemType.substring(0, itemType.indexOf("<Nullable"));
        inner = inner.substring(1, inner.length - 1);
        // TODO 各种泛型
        return `${outer}<${getTypeString(inner)} | null>`;
      } else if (/<((\w+\[\])|\w+)>/.test(itemType)) {
        let inner = /<((\w+\[\])|\w+)>/.exec(itemType)[0];
        let outer = itemType.substring(0, itemType.indexOf("<"));
        inner = inner.substring(1, inner.length - 1);
        // TODO 各种泛型
        return `${outer}<${getTypeString(inner)}>`;
      }
      return itemType;
    } else {
      if (property.additionalProperties) {
        let genericType = getPropertyStrFromAdditionalProperties(
          property.additionalProperties
        );
        return `Record<string, ${genericType}>`;
      }
      return getTypeString(property.type, property.format);
    }
  };

  function getPropertyStrFromAdditionalProperties(additionalProperties) {
    if (additionalProperties.type == "array") {
      return getPropertyTypeStr(additionalProperties);
    } else {
  ...
  const fd = fs.openSync(
    path.join(path.dirname(__dirname), "src", "defs", "Model.ts"),
    "w"
  );
  const importEntities = apiModels
    .filter((m) => m.isEnum !== true)
    ...
    .concat(
      apiModels
        .filter((m) => m.isEnum === true && entityNames.includes(m.name))
        .map((m) => m.name)
    );
  fs.writeSync(fd, `import { MomentInput } from "moment";${os.EOL}`);
  if (importEntities.length > 0) {
    fs.writeSync(
      fd,
      `import { ${importEntities.join(", ")} } from "@/defs/Entity";${os.EOL}${
        os.EOL
      }`
    );
  }
...
`
  ...
  function getGenericClassName(name) {
    const leftIdx = name.indexOf("<");
    const rightIdx = name.lastIndexOf(">");
    if (leftIdx > -1 && rightIdx > -1) {
      return `${name.substring(0, leftIdx)}<T>`;
    }
    return name;
  }
  apiModels
    ...
    .forEach((m) => {
      // TODO 属性中的泛型
      const className = getGenericClassName(m.name);
      if (modelNames.includes(className)) {
        return;
      }
...
`
      ...
      if (m.isEnum) {
        fs.writeSync(
          fd,
          `export declare type ${m.name} = ${m.enum
            .map((val) => `"${val}"`)
            .join(" | ")};${os.EOL}`
        );
      } else {
        fs.writeSync(fd, `export declare class ${className} {${os.EOL}`);
...
`
          ...
          const nullable =
            ...
            !m.required.includes(propertyName) ||
            property.nullable === true;
          fs.writeSync(
            fd,
            `    ${propertyName}${nullable ? "?" : ""}: ${getPropertyTypeStr(
              property
            )};${os.EOL}`
          );
        });
