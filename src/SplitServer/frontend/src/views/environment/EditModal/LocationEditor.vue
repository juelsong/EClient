<template>
  ...
  >
    ...
    >
      <el-form-item :label="$t('Location.editor.Name')" prop="Name">
        <el-input
          ref="modelInnerName"
...
import { oDataPatch, oDataQuery } from "@/utils/odata";
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    async function existSameName() {
      if (props.createNew) {
        const approvalEntityResult = await oDataQuery("Location", {
          $select: "Id, Name",
          $filter: `(IsActive eq true) and (Name eq '${modelInner.value.Name}')`,
        });
        return approvalEntityResult.value.length > 0;
      } else {
        const approvalEntityResult = await oDataQuery("Location", {
          $select: "Id, Name",
          $filter: `(IsActive eq true) and (Id ne ${props.model?.Id}) and (Name eq '${modelInner.value.Name}')`,
        });
        return approvalEntityResult.value.length > 0;
      }
    }
