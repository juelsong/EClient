<template>
  <el-dialog
    ...
    :title="`${$t('Settings.importExport.button.Import')}-预警信息`"
    width="500px"
  >
    <template #footer>
    ...
    async function onImportClick(file) {
      ...
      try {
        const res = await request({
          url: `/Import/excel/EquipmentNotification`,
          method: "post",
          data: formData,
          headers: {
        ...
        if ((res as any).code == 0) {
          ...
          ElMessage.success(i18n.global.t(res.data)); 
          // ensureId.value = res.data;
        } else {
          uploadResult.value = "error";
          errorInfo.value = (res as any).message;
